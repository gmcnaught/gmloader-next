 ...
 * digit lookup table (OSD FPS Overlay feature). The backend emit path can't be
 * unit-tested here (pulls in the full mfgpu backend), so only the branch-worthy
 * logic is factored out and exercised directly, mirroring the Solarus port's
 * tests/fps_overlay_test.c.
 *
 * Build+run: make -f Makefile.gmloader fps-overlay-test (from repo root).
 */
