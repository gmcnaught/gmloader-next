...
- To find which static GL symbols Mesa can't provide: run with `GMLOADER_DBG_THUNKS=1`. `resolve_thunked` (`thunks/thunk_gen_dyn.h`) then prints `DBG resolve NULL: <symbol>` per failure; filter with `grep "DBG resolve NULL" | grep -v "AMD\|ANGLE\|APPLE\|EXT\|NV\|OES"` to see only core function failures. **It is off by default on purpose** — Mesa fails ~679 of these, stderr is unbuffered, and on the MiSTer that log lands on the exFAT SD card at ~1.3 ms per write, which measurably delayed the core's first drawn frame. Unset, you still get one `GL thunks (gles2): N symbol(s) unresolved` line per resolver pass.
